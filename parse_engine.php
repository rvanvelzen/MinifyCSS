...
class parse_engine {
	...
	function eat($type, $semantic, $line = null) {
		...
		case 'e':
			...
			if ($this->enter_error_tolerant_state() || $this->has_step_for('error')) {
				...
				throw new parse_error('Parse Error: ' . $this->descr($type, $semantic) . ' not expected, expected {' . implode(', ', $expected) . '}' . ($line !== null ? ' on line ' . $line : ''));
			}
			break;
		default:
